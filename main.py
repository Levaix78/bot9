import os , requests , telebot , telebot ; from telebot import types ; from uuid import uuid4 ; ud = str(uuid4()) ; from user_agent import generate_user_agent ; gd = str(generate_user_agent());import random;gmail1 = ['gmail', 'gmail1', 'gmail2']
	...
	for user in file:
		try:
			...
			if '_' in user:
							...
							zr0=types.InlineKeyboardButton(text='- {}'.format(email),callback_data='zr0')
							...
							Ronaldo.add(zr0,zr3,zr2,zr1,zr4,zr5)
						...
						if insta(email) == 'ok':
							if gmail(user) == 'ok':
								...
								bot.send_message(message.chat.id,info(user,hit))
							else:
								unava+=1
								num+=1
								Ronaldo = types.InlineKeyboardMarkup(row_width=1)
